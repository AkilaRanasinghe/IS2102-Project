			...
			echo"<div class='row'>
				<div id='Open' class='tabcontent'>
					<div class='row'>";
						...
						$has = false;
						$sql="SELECT DISTINCT * FROM (SELECT username, name AS 'Oname', user_type FROM organiser UNION SELECT username, name AS 'Oname', user_type FROM federation) a, tournament t WHERE CONCAT(a.Oname, '', t.name, '', t.conditions, '', t.venue, '', t.rules_regulations) LIKE '%" . $search . "%' AND a.username = t.username AND (t.entry_closing_date >= (SELECT CURRENT_DATE()) AND t.participants < t.max_participants) AND (t.sport = 'ALL' OR t.sport IN (SELECT sport FROM player_sport WHERE username = '" .$uname. "')) AND t.status = 'Approved'";
				...
				<div class='row'>
					<div id='Closed' class='tabcontent'>
						...
						$has = false;
						$sql="SELECT DISTINCT * FROM (SELECT username, name AS 'Onmae', user_type FROM organiser UNION SELECT username, name AS 'Onmae', user_type FROM federation) a, tournament t WHERE CONCAT(a.Onmae, '', t.name, '', t.conditions, '', t.venue, '', t.rules_regulations) LIKE '%" . $search . "%' AND a.username = t.username AND (t.entry_closing_date < (SELECT CURRENT_DATE()) OR t.participants = t.max_participants) AND (t.sport = 'ALL' OR t.sport IN (SELECT sport FROM player_sport WHERE username = '" .$uname. "')) AND t.status = 'Approved'";
						$result = $conn->query($sql);
