			...
			echo"<div class='row'>
				<div id='Open' class='tabcontent'>
					<div class='row'>";
						...
						$has = false;
						$sql="SELECT DISTINCT ev.occasion_id, ev.name, ev.max_participants, ev.participants, ev.venue, ev.held_date, ev.entry_closing_date, ev.start_time, ev.end_time, ev.expenses_fees, a.username, a.Oname, a.user_type FROM (SELECT username, CONCAT(f_name, ' ', l_name) AS 'Oname', user_type FROM player UNION SELECT username, CONCAT(f_name, ' ', l_name) AS 'Oname', user_type FROM coach UNION SELECT username, name AS 'Oname', user_type FROM organiser UNION SELECT username, name AS 'Oname', user_type FROM federation) a, event ev, event_aspect ea, event_sport es WHERE CONCAT(a.Oname, '', ev.name, '', ev.conditions, '', ev.venue, '', ev.event_type, '', ea.aspect) LIKE '%" . $search . "%' AND ev.username = a.username AND ev.occasion_id = ea.occasion_id AND ev.occasion_id = es.occasion_id AND (ev.entry_closing_date >= (SELECT CURRENT_DATE()) AND ev.participants < ev.max_participants) AND (es.sport = 'ALL' OR es.sport IN (SELECT sport FROM player_sport WHERE username = '" .$uname. "')) AND ev.status = 'Approved'";
				...
				<div class='row'>
					<div id='Closed' class='tabcontent'>
						...
						$has = false;
						$sql="SELECT DISTINCT ev.occasion_id, ev.name, ev.max_participants, ev.participants, ev.venue, ev.held_date, ev.entry_closing_date, ev.start_time, ev.end_time, ev.expenses_fees, a.username, a.Oname, a.user_type FROM (SELECT username, CONCAT(f_name, ' ', l_name) AS 'Oname', user_type FROM player UNION SELECT username, CONCAT(f_name, ' ', l_name) AS 'Oname', user_type FROM coach UNION SELECT username, name AS 'Oname', user_type FROM organiser UNION SELECT username, name AS 'Oname', user_type FROM federation) a, event ev, event_aspect ea, event_sport es WHERE CONCAT(a.Oname, '', ev.name, '', ev.conditions, '', ev.venue, '', ev.event_type, '', ea.aspect) LIKE '%" . $search . "%' AND ev.username = a.username AND ev.occasion_id = ea.occasion_id AND ev.occasion_id = es.occasion_id AND (ev.entry_closing_date < (SELECT CURRENT_DATE()) OR ev.participants = ev.max_participants) AND (es.sport = 'ALL' OR es.sport IN (SELECT sport FROM player_sport WHERE username = '" .$uname. "')) AND ev.status = 'Approved'";
						$result = $conn->query($sql);
