	...
	<div class="column contents">
		...
		<div class="maintabcontent">
			<div class="row tab" style="padding-left:0px;">
				<a class="buttonlinks" onclick="openUser(event, 'Find')" id="defaultOpen" style="width:25%;">Find Partners</a>
				<a class="buttonlinks" onclick="openUser(event, 'Requested')" style="width:25%;">Requested Partners</a>
				<a class="buttonlinks" onclick="openUser(event, 'Partners')" style="width:25%;">My Partners</a>
				<a class="buttonlinks" onclick="openUser(event, 'My')" style="width:25%;">My Requests</a>
			</div>
			<?php
			$search = "";
					...
					<div class='row'>";
						...
						if($resultt->num_rows > 0)
						{
							while($roww = $resultt->fetch_assoc())
							{
								...
								$sqli="SELECT status FROM player_request WHERE (requester_username = '" .$uname. "' AND receiver_username = '" .$playerusername. "') OR (requester_username = '" .$playerusername. "' AND receiver_username = '" .$uname. "')";
								...
								if($resulti->num_rows < 1)
								{
									echo"<div class='item'>
										...
										<a href='viewplayer.php?id=".$playerusername."' style='text-decoration: none; color:white;'><h3 style='margin-bottom:5px;'>".$roww["f_name"]." ".$roww["l_name"]."</h3></a>									
										<p>".$roww["achievement_level"]."</p>
										<a href='../php/requestplayer.php?id=".$playerusername."'><button>Send Request</button></a>
									</div>";					
								}
							}
						}
				...
				<div class='row'>
					<div id='Requested' class='tabcontent'>
						...
						if($resultt->num_rows > 0)
						{
							...
							{
								$playerusername = $roww["username"];
								$sqli="SELECT status FROM player_request WHERE requester_username = '" .$uname. "' AND receiver_username = '" .$playerusername. "'";
								$resulti = $conn->query($sqli);
								...
								if($resulti->num_rows > 0)
								{
									while($rowi = $resulti->fetch_assoc())
									{
										...
										if($status == "Pending")
										{
											echo"<div class='item'>
												<img src='data:image/jpeg;base64," . base64_encode($roww['picture']) . "' class='imge' alt='Avatar'>
												<a href='requestedplayer.php?id=".$playerusername."' style='text-decoration: none; color:white;'><h3 style='margin-bottom:5px;'>".$roww["f_name"]." ".$roww["l_name"]."</h3></a>									
												<p>".$roww["achievement_level"]."</p>
												<a href='../php/removeplayer.php?id=".$playerusername."'><button>Undo Request</button></a>
											</div>";
										}
									}
								}							
				...
				<div class='row'>
					<div id='My' class='tabcontent'>
						...
						if($resultt->num_rows > 0)
						{
							...
							{
								$playerusername = $roww["username"];
								$sqli="SELECT requester_username, status FROM player_request WHERE requester_username = '" .$playerusername. "' AND receiver_username = '" .$uname. "'";
								$resulti = $conn->query($sqli);
